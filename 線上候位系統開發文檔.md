...
1. **候位狀態顯示**
   ...
   - 顯示等待人數
   - 顯示預估開始時間（根據下次辦事時間加上等待人數*13分鐘計算，並顯示上下午格式）
   - 顯示下次辦事時間（若目前沒有候位）
...
1. **取得目前候位狀態**
   ...
   - 回傳：目前叫號、等待人數、預估等待時間、預估開始時間、下次辦事時間
...
3. **查詢候位狀態**
   ...
   - 回傳：該號碼的候位狀態、前面等待人數、預估等待時間

4. **通過姓名和電話查詢候位號碼**
   - GET `/api/queue/search?name=姓名&phone=電話`
   - 回傳：候位號碼、候位狀態、前面等待人數、預估開始時間
...
- 計算公式：預估結束時間 = 下次辦事時間 + (等待人數 × 每位客戶預估處理時間)
...
系統在開發過程中遇到並解決了關鍵的技術問題：

#### 1. Lunar-JavaScript API 使用問題
**問題描述**：
...
本文檔提供了線上候位系統的完整開發指南，包括實際實現的功能、技術架構、數據模型、API設計、頁面設計以及部署指南。系統已實現了基本的候位管理、自動計算預估時間、以及狀態重置功能，確保用戶體驗流暢。

系統特別適合需要管理現場候位的場景，通過網頁形式提供服務，無需下載App，大大提升了使用便利性和效率。在部署方面，已解決了 Windows 系統的端口衝突問題，提供了完整的故障排除指南，確保系統能在各種環境下穩定運行。 
...
**變更背景**：  
候位登記功能已從公開服務調整為僅管理員可用，這是為了更好地控制候位流程和管理需求。

**技術實施**：
- **前端限制**：首頁和導航欄的"我要候位"按鈕僅登入管理員可見
- **路由保護**：`/register` 路由使用 `ProtectedRoute` 包裝，需要管理員認證
- **後台整合**：AdminDashboardPage 新增「登記候位」浮動視窗功能
- **組件重用**：使用現有的 RegisterForm 組件，確保功能一致性

**核心修改檔案**：
- `frontend/src/pages/HomePage.jsx` - 添加認證條件判斷
- `frontend/src/components/Layout.jsx` - 導航欄按鈕認證限制
- `frontend/src/App.js` - register 路由添加 ProtectedRoute 保護
- `frontend/src/pages/admin/AdminDashboardPage.jsx` - 新增登記候位浮動視窗

**架構完整保留**：
- 所有候位登記相關的組件、API、驗證邏輯完全保留
- RegisterForm 組件功能完整不變
- 後端 API 端點和資料處理邏輯維持原狀
- 日後重新開放僅需移除認證條件和路由保護

**管理員操作流程**：
1. 登入管理後台 (`/admin/login`)
2. 進入候位管理頁面 (`/admin/dashboard`)
3. 點擊「登記候位」按鈕（綠色按鈕，PersonAddIcon 圖示）
4. 在 Dialog 浮動視窗中完成客戶候位登記
5. 登記成功後自動關閉視窗並刷新候位列表

**未來重新開放準備**：
當需要重新開放給公眾使用時，僅需進行以下簡單修改：
- 移除 HomePage.jsx 中的 `{isAuthenticated && (...)}`條件
- 移除 Layout.jsx 中的認證條件判斷
- 移除 App.js 中 register 路由的 `<ProtectedRoute>` 包裝
- 推送更新即可立即重新開放
...
2. **候位登記功能（暫時關閉，僅管理員可用）**
   - ⚠️ **重要變更**：此功能已暫時關閉，一般訪客無法直接使用
   - **技術實現**：路由受 ProtectedRoute 保護，需要管理員認證
   - **管理員專用**：透過後台「登記候位」浮動視窗進行客戶登記

...
### 📋 其他詳細內容功能（2024年12月新增）
**功能說明**: 當客戶在諮詢主題中選擇「其他」時，系統會顯示詳細內容輸入欄位，讓客戶可以說明具體的諮詢問題。  
**適用範圍**: 所有候位登記和客戶資料修改介面，包括前台登記、後台管理員登記、客戶自助編輯等。  
**技術實施**:
- 後端模型新增 `otherDetails` 欄位（字串型態，最大500字符）
- 前端表單動態顯示詳細內容輸入框（僅在選擇「其他」時出現）
- 管理後台完整支援其他詳細內容的顯示和編輯
- 資料匯出功能包含詳細內容，格式為「其他(詳細內容)」
- **簡化模式兼容**: 在簡化模式下，即使選擇「其他」也不強制要求填寫詳細內容

**使用說明**:
- 客戶在任何登記介面選擇「其他」諮詢主題時，會自動顯示多行文字輸入框
- 標準模式下：必須填寫詳細內容才能提交表單
- 簡化模式下：詳細內容為選填，系統會自動提供預設值「簡化模式快速登記」
- 管理員在後台可查看、編輯客戶的詳細內容
- 匯出資料時會以「其他(客戶填寫的詳細內容)」格式顯示
...
- **地址資訊**：預設地址和地址類型
- **諮詢主題**：預設選擇「其他」
- **其他詳細內容**：當自動選擇「其他」時，提供預設值「簡化模式快速登記」

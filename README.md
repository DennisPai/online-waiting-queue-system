...
- **智慧預估時間計算**：全新優化的時間預估系統，提供更準確的等待時間
  - **首頁固定預估**：基於總客戶數×每位處理時間，顯示今日預估結束時間
  - **個人動態預估**：基於上一位辦完時間+(叫號順序-1)×每位處理時間，精確計算輪到時間
  - **管理員控制**：可手動設定總客戶數和上一位辦完時間，並提供一鍵重設功能
...
- **一鍵叫號功能**：自動標記完成、更新時間、遞補排序，並即時更新首頁顯示
- 靈活管理候位清單
- 客戶資料管理與篩選功能
- 設定下次辦事時間
- 候位順序拖曳調整功能

...
## ⚠️ 重要功能變更（同步自 `docs/TODO.md` 已驗收項目）

### 公開候位登記開關功能
### v1 API 與前端整合（最新）
- 後端新增 `/api/v1` 並統一回應格式 `{ success, code, message, data }`
- `auth/login` 回傳 `data: { user, token }`（token 移入 data）
- `/ready` 就緒檢查回傳 Mongo 連線狀態（200/503）
- 前端 `API_VERSION=v1` 預設；`services/*` 與 `slices` 單層 data 解包
- 取消首次登入強制改密，改為管理員「手動變更密碼」
**生效日期**: 2024年12月  

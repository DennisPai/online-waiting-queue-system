...
**背景**：用戶報告匯出資料按鈕無反應，並要求檢查重新排序功能。

**問題分析**：

1. **匯出資料按鈕無反應**
   - **症狀**：後台點擊「匯出資料」按鈕沒有任何反應
   - **根本原因**：
     - `AdminDashboardPage` 使用 `handleExport` 方法（第 103、163 行）
     - `useQueueUI` 只導出 `handleOpenExportDialog` 方法（第 168 行）
     - 函數名稱不匹配導致 `handleExport` 為 `undefined`
   - **解決方案**：在 `useQueueUI` 的返回值中添加 `handleExport` 別名

2. **重新排序功能性能和錯誤處理問題**
   - **症狀**：功能可用但有改進空間
   - **潛在問題**：
     - 使用 `for` 循環逐個發送請求，客戶數量多時很慢
     - 錯誤處理不完整，中途失敗會導致數據不一致
     - 沒有在成功後重新載入列表，可能導致前後端狀態不同步
   - **解決方案**：
     - 使用 `Promise.all` 並行發送所有請求
     - 成功/失敗都重新載入列表，確保數據一致性
     - 使用 `unwrap()` 確保錯誤正確捕獲
...
2. **優化重新排序功能**（[`frontend/src/hooks/admin/useQueueActions.js`](frontend/src/hooks/admin/useQueueActions.js)）：
   ```javascript
...
**性能改進**：
- **重新排序速度**：假設有 50 個客戶，每個請求耗時 100ms
  - 舊版本（循環）：50 × 100ms = 5000ms (5秒)
  - 新版本（並行）：100ms (只需一次往返時間)
  - 性能提升：約 50 倍

**經驗教訓**：
1. **函數命名一致性**：確保導出的函數名稱與使用端一致，或提供別名
2. **並行處理**：批量操作應優先使用 `Promise.all` 而非循環
3. **數據一致性**：操作完成後重新載入是確保前後端同步的最可靠方式
4. **錯誤恢復**：失敗時應恢復到已知的正確狀態

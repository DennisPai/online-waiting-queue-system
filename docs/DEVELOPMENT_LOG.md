...
1. **雙模式設計**
   - 預設模式：使用系統自動計算（開科辦事日 + 1天 + 中午12:00整）
   - 自訂模式：管理員可以輸入任何日期時間文字
...
1. **後端資料模型與 API**：
   - [`backend/src/models/system-setting.model.js`](backend/src/models/system-setting.model.js)
     - 新增 `nextRegistrationDateTime` 欄位（String, default: null）
     - null 表示使用動態計算，字串表示使用自訂值
   
   - [`backend/src/controllers/admin.controller.js`](backend/src/controllers/admin.controller.js)
     - 新增 `getNextRegistrationDateTime` 控制器函數
     - 新增 `updateNextRegistrationDateTime` 控制器函數
     - 使用標準 `async/await + try-catch` 錯誤處理模式
   
   - [`backend/src/routes/v1/admin.routes.js`](backend/src/routes/v1/admin.routes.js)
     - 新增 `GET /api/v1/admin/settings/next-registration-datetime` 路由
     - 新增 `PUT /api/v1/admin/settings/next-registration-datetime` 路由
   
   - [`backend/src/controllers/queue.controller.js`](backend/src/controllers/queue.controller.js)
     - 更新 `getQueueStatus` 函數，在返回資料中加入 `nextRegistrationDateTime`
     - 確保前端重新整理時能獲取最新設定

2. **前端 Redux 狀態管理**：
   - [`frontend/src/redux/slices/queueSlice.js`](frontend/src/redux/slices/queueSlice.js)
     - 在 initialState 中新增 `nextRegistrationDateTime: null`
     - 新增 `getNextRegistrationDateTime` async thunk
     - 新增 `setNextRegistrationDateTime` async thunk
     - 在 `getQueueStatus.fulfilled` 中處理 `nextRegistrationDateTime` 同步
     - 新增對應的 fulfilled/rejected 處理器
   
   - [`frontend/src/services/queueService.js`](frontend/src/services/queueService.js)
     - 新增 `getNextRegistrationDateTime` 服務函數
     - 新增 `setNextRegistrationDateTime` 服務函數
     - 統一處理 v1 API 回應格式

3. **後台管理介面**：
   - [`frontend/src/pages/admin/AdminSettingsPage.jsx`](frontend/src/pages/admin/AdminSettingsPage.jsx)
     - 引入 `getNextRegistrationDate` 工具函數用於動態計算
     - 新增 `nextRegistrationDateTime` 和 `useCustomDateTime` 狀態
     - 新增 `getDefaultDateTime()` 輔助函數計算預設值
     - 新增 `handleToggleCustomDateTime()` 處理模式切換
     - 新增 `handleSaveNextRegistrationDateTime()` 處理儲存
     - 在「註冊設定」Tab 中新增完整的設定介面：
       - 模式切換開關（Switch）
       - 文字輸入框（TextField，自訂模式時啟用）
       - 說明提示（Alert）
       - 即時預覽（Alert）
       - 儲存按鈕（Button）
     - 保留「公開候位登記設置」區塊
...
4. **前台顯示更新**：
   - [`frontend/src/pages/HomePage.jsx`](frontend/src/pages/HomePage.jsx)
     - 新增 `getDisplayDateTime()` 函數
     - 優先使用 `queueStatus.nextRegistrationDateTime`（自訂值）
     - 否則使用 `getNextRegistrationDate()` 動態計算
     - 更新額滿提示訊息顯示邏輯
   
   - [`frontend/src/pages/RegisterPage.jsx`](frontend/src/pages/RegisterPage.jsx)
     - 新增相同的 `getDisplayDateTime()` 函數
     - 更新額滿提示訊息顯示邏輯

5. **文檔更新**：
   - [`docs/API_SPEC.md`](docs/API_SPEC.md)
     - 更新 `GET /api/v1/queue/status` 回應格式說明
     - 新增 `GET /api/v1/admin/settings/next-registration-datetime` API 規格
     - 新增 `PUT /api/v1/admin/settings/next-registration-datetime` API 規格
     - 詳細說明 null 和字串值的含義

**驗收結果**：

- [x] 後台「註冊設定」分頁出現「候位額滿提示設定」區塊
- [x] 預設顯示「使用系統自動計算」模式
- [x] 顯示動態計算的日期（開科辦事日+1天中午12:00整）
- [x] 可以切換為「使用自訂時間」並輸入自訂文字
- [x] 切換回「使用系統自動計算」時恢復顯示動態計算值
- [x] 儲存後立即生效，首頁和報名頁正確更新
- [x] 重新整理頁面後設定不會丟失
- [x] 自訂值和預設計算都能正確顯示
- [x] API 文檔已更新
- [x] 開發日誌已記錄
...
1. **資料持久化完整路徑**：
   - 需要確保資料從資料庫 → API → Redux → UI 的完整流程
   - 任何一個環節缺失都會導致資料丟失或不同步

2. **預設值處理的最佳實踐**：
   - 使用 `null` 表示「使用預設邏輯」比使用固定字串更靈活
   - 允許系統保留動態計算能力，同時提供覆蓋選項
   - 在多個層級（Model、Controller、Redux）都設定預設值確保容錯

3. **保留現有邏輯避免破壞性變更**：
   - `getNextRegistrationDate` 函數繼續使用，不需要重寫
   - 前端判斷邏輯簡單明確：有自訂值用自訂值，否則用計算值
   - 向後兼容，不影響現有功能

4. **雙模式設計的 UX 考量**：
   - 提供開關讓使用者清楚知道當前使用的模式
   - 預設模式時禁用輸入框，避免誤解
   - 提供即時預覽，讓使用者在儲存前就能看到效果
   - 說明文字清楚標示兩種模式的差異

5. **API 設計的一致性**：
   - 遵循 v1 API 規範：`{ success, code, message, data }`
   - 使用 kebab-case 命名：`next-registration-datetime`
   - 錯誤處理模式與檔案其他函數保持一致（`async/await + try-catch`）

6. **文檔同步的重要性**：
   - API 變更時立即更新 API_SPEC.md
   - 詳細記錄新增或修改的欄位
   - 在開發日誌中記錄設計決策和經驗教訓
   - 幫助團隊成員和 AI Agent 理解變更

**相關 Commit**：
- `feat: 新增候位額滿提示訊息自訂功能（支援動態計算和自訂模式）`

---

### 2025-11-30 - 修正活動報名設定持久化問題

**背景**：用戶反映當重新整理網頁時，原本「活動報名」設定好的內容都會回歸預設，活動報名功能無法正常使用。

**問題診斷**：

經過詳細檢查，發現根本原因：

1. **後端 getQueueStatus API 缺少 eventBanner 欄位**
   - `GET /api/v1/queue/status` 是所有頁面載入時的核心 API
   - 該 API 返回了其他系統設定（isOpen, maxOrderIndex, publicRegistrationEnabled 等）
   - 但沒有返回 `eventBanner` 資料
   - 導致頁面重新整理時無法獲取 eventBanner 設定

2. **前端 Redux 未處理 eventBanner 同步**
   - `getQueueStatus.fulfilled` 處理器更新了其他狀態
   - 但沒有更新 `state.eventBanner`
   - 即使後端有返回，前端也不會更新到 Redux 狀態

3. **首頁輪詢機制導致設定丟失**
   - 首頁每 5 分鐘自動調用 `getQueueStatus()` 更新資料
   - 由於上述兩個問題，每次更新都不會同步 eventBanner
   - 但其他狀態會更新，導致資料不一致

4. **前端服務層函數未實作**
   - 首頁嘗試調用 `dispatch(getEventBanner())`
   - 但 `queueService.js` 中沒有實作 `getEventBanner` 函數
   - 雖然 Redux thunk 已定義，但缺少底層服務函數

**技術實施**：

1. **後端加入 eventBanner 到 getQueueStatus**：
   - [`backend/src/controllers/queue.controller.js`](backend/src/controllers/queue.controller.js)
     - 在系統關閉時的回應中加入 `eventBanner: settings.eventBanner`（第 182 行）
     - 在系統開啟時的回應中加入 `eventBanner: settings.eventBanner`（第 237 行）
     - 確保所有情況下都返回完整的 eventBanner 資料

2. **前端 Redux 處理 eventBanner 同步**：
   - [`frontend/src/redux/slices/queueSlice.js`](frontend/src/redux/slices/queueSlice.js)
     - 在 `getQueueStatus.fulfilled` 處理器中加入邏輯（第 481-483 行）
     - 當 `action.payload.eventBanner` 存在時更新 `state.eventBanner`
     - 確保每次調用 getQueueStatus 都會同步 eventBanner

3. **前端服務層實作 getEventBanner**：
   - [`frontend/src/services/queueService.js`](frontend/src/services/queueService.js)
     - 實作 `getEventBanner` 函數（第 397-403 行）
     - 調用 `GET /api/v1/admin/settings/event-banner` API
     - 處理 v1 API 回應格式，返回 `response.data.data || response.data`
     - 加入匯出列表（第 528 行）

4. **首頁優化移除重複調用**：
   - [`frontend/src/pages/HomePage.jsx`](frontend/src/pages/HomePage.jsx)
     - 移除單獨的 `dispatch(getEventBanner())` 調用
     - 移除未使用的 `getEventBanner` import
     - 因為 `getQueueStatus()` 已包含 eventBanner，避免重複請求

5. **API 文檔更新**：
   - [`docs/API_SPEC.md`](docs/API_SPEC.md)
     - 更新 `GET /api/v1/queue/status` 的回應格式說明
     - 詳細列出 eventBanner 物件的所有屬性
     - 加入說明：確保前端重新整理時不會丟失設定

**驗收結果**：

- [x] 重新整理頁面時，活動報名設定保持不變
- [x] 首頁每 5 分鐘自動更新時，不會影響 eventBanner 顯示
- [x] 管理員儲存後，立即在首頁看到變更
- [x] 多個分頁同時開啟時，資料保持一致
- [x] API 規格文檔已更新（`docs/API_SPEC.md`）
- [x] 所有修改符合工程規範（v1 API 格式、錯誤處理）

**經驗教訓**：

1. **核心 API 應返回完整狀態**：
   - `getQueueStatus` 作為系統狀態的核心 API
   - 應該返回所有必要的系統設定，避免遺漏
   - 新增系統設定時，記得同步更新此 API
...
3. **避免重複 API 調用**：
   - 當核心 API 已包含資料時，不需要額外調用
   - 減少網路請求，提升效能
   - 保持資料來源單一，避免不一致

4. **服務層函數完整性**：
   - Redux thunk 定義後，確保對應的服務層函數已實作
   - 檢查函數是否已加入匯出列表
   - 避免執行時才發現函數未定義
...
**相關 Commit**：
- `a62a838` - fix: 修正活動報名設定持久化問題並更新API文檔

**預防措施**：

- 新增系統設定時，檢查清單：
  1. ✓ 資料庫 Model 定義
  2. ✓ 後端 API 端點（GET/PUT/POST）
  3. ✓ getQueueStatus 包含該設定
  4. ✓ 前端 Redux state 定義
  5. ✓ 前端 Redux reducer 處理
  6. ✓ 前端服務層函數實作
  7. ✓ API_SPEC.md 文檔更新
